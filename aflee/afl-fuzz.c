...
EXP_ST u64 cycles_number = DEF_CYCLES_NUMBER;
...
EXP_ST u8* trace_bits;                /* SHM with instrumentation   */
...
static void backup_folder(char *folder_name) {
  time_t cur_t = time(0);
  struct tm* t = localtime(&cur_t);

#ifndef SIMPLE_FILES

  u8* new_folder_name = alloc_printf("%s.%04u-%02u-%02u-%02u:%02u:%02u",
                                  folder_name, t->tm_year + 1900,
                                  t->tm_mon + 1, t->tm_mday, t->tm_hour,
                                  t->tm_min, t->tm_sec);

#else

  u8* new_folder_name = alloc_printf("%s_%04u%02u%02u%02u%02u%02u", folder_name,
...
static u8 run_target(char** argv) {
  ...
  /* $KM$ */
  if (klee_mode && WEXITSTATUS(status) == TASSERT_MAGIC) {
    OKF("Bingo!");
    return TASSERT_HIT;
  }
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  switch (fault) {
    ...
    case TASSERT_HIT:
      ...
      fn = alloc_printf("%s/tasserts/id:%06llu", out_dir, tassert_hits);

#else

      fn = alloc_printf("%s/tasserts/id_%06llu", out_dir, tassert_hits);
...
static void write_stats_file(double bitmap_cvg, double eps) {
  ...
  fprintf(f, "start_time     : %llu\n"
             ...
             tassert_hits, last_path_time / 1000, last_crash_time / 1000,
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  static u64 prev_qc, prev_uc, prev_uh, prev_ta;

  if (prev_qp == queued_paths && prev_pf == pending_favored &&
      prev_pnf == pending_not_fuzzed && prev_ce == current_entry &&
      prev_qc == queue_cycle && prev_uc == unique_crashes &&
      prev_uh == unique_hangs && prev_ta == tassert_hits &&
      prev_md == max_depth) return;
  ...
  fprintf(plot_file,
          "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %llu, %u,\n"
          "%0.02f\n",
          get_cur_time() / 1000, queue_cycle - 1, current_entry, queued_paths,
          pending_not_fuzzed, pending_favored, bitmap_cvg, unique_crashes,
          tassert_hits, unique_hangs, max_depth, eps); /* ignore errors */
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s/tasserts", out_dir);
...
EXP_ST void setup_dirs_fds(void) {
  ...
  tmp = alloc_printf("%s/tasserts", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
  ...
  fprintf(plot_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     ...
                     "unique_hangs, tassert_hits, max_depth, execs_per_sec\n");
                     /* ignore errors */
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnKCB:S:M:x:c:QZ:")) > 0)
    ...
    switch (opt) {
      ...
      case 'c': {
        ...
        if (!strcmp(optarg, "def")) {
          cycles_number = DEF_CYCLES_NUMBER;
          break;
        }
        if (sscanf(optarg, "%llu%c", &cycles_number, &suffix) < 1
            || optarg[0] == '-') {
  ...
  setup_signal_handlers();  // signal handlers are defined in afl-fuzz.c

  // check ASAN_OPTIONS and MSAN_OPTIONS environment variable
  check_asan_opts();
  ...
  if (dumb_mode) {
    ...
    if (klee_mode)  FATAL("-K and -n are mutually exclusive");
  ...
  // copy command line arguments to orig_cmdline
  save_cmdline(argc, argv);

  // write name of the executed program to use_banner
  ...
  setup_post();
  setup_shm();  // pointer to SHM region is written to shm_id
  ...
  // checking that the file exists, is executable and instrumented
  ...
  while (1) {
    ...
    if (!queue_cur) {
      ...
      if (klee_mode && cycles_limit) {
        if (queued_paths == prev_queued && queue_cycle >= cycles_number) {
          OKF("No new paths have been discovered during %llu cycles. "
                  "Finishing...", queue_cycle);
          goto stop_fuzzing;
        }
      }

      if (klee_mode && tassert_hits >= 100) {
        OKF("Too many terminal asserts hit. Finishing...");
        goto stop_fuzzing;
      }
